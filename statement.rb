...
@query_activity_joined_users_by_activity_id = @client.prepare("SELECT *,us.agent_id,us.telegram_id,us.telegram_username,us.authority,ad.duty_name FROM activity_users AS au LEFT JOIN users AS us ON au.telegram_id=us.telegram_id LEFT JOIN activity_duty AS ad ON au.duty=ad.id LEFT JOIN  authority_name AS an ON us.authority=an.id WHERE au.activity_id=? AND ad.id>2")
